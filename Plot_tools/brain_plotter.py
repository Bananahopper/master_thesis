...
class BrainPlotter():
    def __init__(self, 
        ...
        self.cort_labels = ["void", "frontal pole", "insular cortex", "superior frontal gyrus", "middle frontal gyrus", "inferior frontal gyrus, pars triangularis",
                    "inferior frontal gyrus, pars opercularis", "precentral gyrus", "temporal pole", "superior temporal gyrus, anterior division",
                    "superior temporal gyrus, posterior division", "middle temporal gyrus, anterior division", "middle temporal gyrus, posterior division",
                    "middle temporal gyrus, temporooccipital part", "inferior temporal gyrus, anterior division", "inferior temporal gyrus, posterior division",
                    "inferior temporal gyrus, temporooccipital part", "postcentral gyrus", "superior parietal lobule", "supramarginal gyrus, anterior division",
                    "supramarginal gyrus, posterior division", "angular gyrus", "lateral occipital cortex, superior division", 
                    "lateral occipital cortex, inferior division", "intracalcarine cortex", "frontal medial cortex", "juxtapositional lobule cortex", 
                    "subcallosal cortex", "paracingulate gyrus", "cingulate gyrus, anterior division", "cingulate gyrus, posterior division", 
                    "precuneous cortex", "cuneal cortex", "frontal oribtal cortex", "parahippocampal gyrus, anterior division", 
                    "parahippocampal gyrus, posterior division", "lingual gyrus", "temporal fusiform cortex, anterior division", 
                    "temporal fusiform cortex, posterior division", "temporal occipital fusiform cortex", "occipital fusiform gyrus", "frontal operculum cortex", 
                    "central opercular cortex", "parietal operculum cortex", "planum polare", "heschls gyrus", "planum temporale", "supracalcarine cortex", 
                    "occipital pole"]
        self.sub_labels = ["void", "left cerebral white matter", "left cerebral cortex", "left lateral ventrical", "left thalamus", "left caudate", 
                           "left putamen", "left pallidum", "brain-stem", "left hippocampus", "left amygdala", "left accumbens", "right cerebral white matter", 
                           "right cerebral cortex", "right lateral ventricle", "right thalamus", "right caudate", "right putamen", "right pallidum", 
                           "right hippocampus", "right amygdala", "right accumbens"]

        self.affine_nib = self.atlas_path_cort if self.atlas_path_cort else self.atlas_path_sub
    ...
    def process(self):
        ...
        if self.task_1:
            ...
            atlas_dict_1 = self.create_atlas_dict(self.atlas_path_cort, self.cort_labels)
        ...
        atlas_nib_result = self.replace_values_in_nib(self.atlas_path, atlas_dict, self.result_path)
    ...
    def replace_values_in_nib(self, atlas_path: list, atlas_dict: list, result_path: list):
        """
    ...
    def save_nib(self, atlas_nib_result: list):
        ...
        for index, atlas_nib in enumerate(atlas_nib_result):
            ...
            nib.save(atlas_nib, f'{self.save_path}/output{index}.nii' )
