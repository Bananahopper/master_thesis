...
# Configure logging
logging.basicConfig(filename='per_dataset_stats.log', level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s')

class PerDatasetDist:
    def __init__(self, directory, name, save_path):
    ...
    def process(self):
        ...
        merged_cortical, merged_subcortical = self.get_statistics(added_dfs_cortical, added_dfs_subcortical)
    ...
    def get_statistics(self, added_dfs_cortical, added_dfs_subcortical):
        ...
        merged_df =  reduce(lambda  left,right: pd.merge(left,right,on=['Labels'], how='outer', suffixes=(None, x)), added_dfs_cortical).fillna(0)
        ...
        merged_df =  reduce(lambda  left,right: pd.merge(left,right,on=['Labels'], how='outer', suffixes=(None, y)), added_dfs_subcortical).fillna(0)
        summed_df = merged_df.sum(axis=1)
    ...
    def save_statistics(self, merged_cortical, merged_subcortical):
        ...
        merged_subcortical.to_csv(self.save_path + f"/subcortical_stats_for_{self.name}.csv")

        logging.info(f"Saved subcortical stats for {self.name}")

        df_cortical = merged_cortical.drop('void')
        df_cortical_sorted = df_cortical.sort_values(by='Tumor in region', axis=0, ascending=False)

        plt.figure()
        df_cortical_sorted.plot(kind='barh', figsize=(25, 20), edgecolor='black', colormap='tab20b')
        plt.xlabel('Percentage of tumor in region')
        plt.title(f'Percentage of tumor in each cortical region for {self.name} dataset')
        plt.legend(loc='center left', bbox_to_anchor=(1.0, 0.5))
        plt.savefig(self.save_path + f"/cortical_plot_for_{self.name}.png")
        plt.close()

        df_subcortical = merged_subcortical.drop('void')
        df_subcortical_sorted = df_subcortical.sort_values(by='Tumor in region', axis=0, ascending=False)

        plt.figure()
        df_subcortical_sorted.plot(kind='barh', figsize=(25, 20), edgecolor='black', colormap='tab20b')
        plt.xlabel('Percentage of tumor in region')
        plt.title(f'Percentage of tumor in each subcortical region for {self.name} dataset')
        plt.legend(loc='center left', bbox_to_anchor=(1.0, 0.5))
...
def main(dir, name, save_path):
    perdata = PerDataset(dir, name, save_path)
    perdata.process()
...
if __name__ == "__main__":
    ...
    main(args.dir, args.name, args.save_path)